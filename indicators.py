...
def acc_dist(data, trend_periods=21, open_col='Open', high_col='High', low_col='Low', close_col='Close', vol_col='Volume'):
    for index, row in data.iterrows():
        if row[high_col] != row[low_col]:
            ac = ((row[close_col] - row[low_col]) - (row[high_col] -
                  row[close_col])) / (row[high_col] - row[low_col]) * row[vol_col]
        else:
            ac = 0
        data.at[index, 'acc_dist'] = ac
    data['acc_dist_ema' + str(trend_periods)] = data['acc_dist'].ewm(
...
def on_balance_volume(data, trend_periods=21, close_col='Close', vol_col='Volume'):
    ...
    data['obv_ema' + str(trend_periods)] = data['obv'].ewm(ignore_na=False,
                                                           min_periods=0, com=trend_periods, adjust=True).mean()
    ...
    for index, row in data.iterrows():
        if index > 0:
            ...
            current_val = last_val + \
                (today_vol * (today_close - last_close) / last_close)
        else:
            current_val = row[vol_col]

        data.at[index, 'pvt'] = current_val
    data['pvt_ema' + str(trend_periods)] = data['pvt'].ewm(ignore_na=False,
                                                           min_periods=0, com=trend_periods, adjust=True).mean()
...
def bollinger_bands(data, trend_periods=20, close_col='Close'):
    ...
    data['bol_bands_middle'] = data[close_col].ewm(
        ignore_na=False, min_periods=0, com=trend_periods, adjust=True).mean()
    for index, row in data.iterrows():
...
def chaikin_oscillator(data, periods_short=3, periods_long=10, high_col='High',
    ...
    for index, row in data.iterrows():
        if row[high_col] != row[low_col]:
            val = val_last + ((row[close_col] - row[low_col]) - (
                row[high_col] - row[close_col])) / (row[high_col] - row[low_col]) * row[vol_col]
        else:
            val = val_last
        ac.at[index] = val
        val_last = val

    ema_long = ac.ewm(ignore_na=False, min_periods=0,
                      com=periods_long, adjust=True).mean()
    ema_short = ac.ewm(ignore_na=False, min_periods=0,
                       com=periods_short, adjust=True).mean()
...
def ease_of_movement(data, period=14, high_col='High', low_col='Low', vol_col='Volume'):
    for index, row in data.iterrows():
        if index > 0:
            midpoint_move = (row[high_col] + row[low_col]) / 2 - \
                (data.at[index - 1, high_col] +
                 data.at[index - 1, low_col]) / 2
        else:
            midpoint_move = 0
    ...
    data['emv_ema_'+str(period)] = data['emv'].ewm(ignore_na=False,
                                                   min_periods=0, com=period, adjust=True).mean()
...
def mass_index(data, period=25, ema_period=9, high_col='High', low_col='Low'):
    # this is to avoid division by zero below
    high_low = data[high_col] - data[low_col] + 0.000001
    ema = high_low.ewm(ignore_na=False, min_periods=0,
                       com=ema_period, adjust=True).mean()
    ema_ema = ema.ewm(ignore_na=False, min_periods=0,
                      com=ema_period, adjust=True).mean()
    div = ema / ema_ema
...
def directional_movement_index(data, periods=14, high_col='High', low_col='Low'):
    ...
    data['di_plus'] = (data['dm_plus'] / data['true_range']).ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
    data['di_minus'] = (data['dm_minus'] / data['true_range']).ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()

    data['dxi'] = np.abs(data['di_plus'] - data['di_minus']) / \
        (data['di_plus'] + data['di_minus'])
    data.at[0, 'dxi'] = 1.
    data['adx'] = data['dxi'].ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
...
def money_flow_index(data, periods=14, vol_col='Volume'):
    ...
    for index, row in data.iterrows():
        ...
        if index >= periods:
            ...
            positive_sum = data['money_flow_positive'][index -
                                                       periods:index].sum()
            negative_sum = data['money_flow_negative'][index -
                                                       periods:index].sum()
    ...
    data = data.drop(['money_flow', 'money_ratio',
                     'money_flow_positive', 'money_flow_negative'], axis=1)
    ...
    for index, row in data.iterrows():
        if index > 0:
            ...
            if row[vol_col] < data.at[index-1, vol_col]:
                nvi = prev_nvi + (row[close_col] -
                                  prev_close / prev_close * prev_nvi)
            else:
                nvi = prev_nvi
        else:
            nvi = 1000
        data.at[index, 'nvi'] = nvi
    data['nvi_ema'] = data['nvi'].ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
...
def positive_volume_index(data, periods=255, close_col='Close', vol_col='Volume'):
    ...
    for index, row in data.iterrows():
        if index > 0:
            ...
            prev_close = data.at[index-1, close_col]
            if row[vol_col] > data.at[index-1, vol_col]:
                pvi = prev_pvi + (row[close_col] -
                                  prev_close / prev_close * prev_pvi)
            else:
                pvi = prev_pvi
        else:
            pvi = 1000
        data.at[index, 'pvi'] = pvi
    data['pvi_ema'] = data['pvi'].ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
...
def rsi(data, periods=14, close_col='Close'):
    ...
    data['rsi'] = data['rsi_u'].ewm(ignore_na=False, min_periods=0, com=periods, adjust=True).mean() / (data['rsi_u'].ewm(ignore_na=False,
                                                                                                                          min_periods=0, com=periods, adjust=True).mean() + data['rsi_d'].ewm(ignore_na=False, min_periods=0, com=periods, adjust=True).mean())
...
def chaikin_volatility(data, ema_periods=10, change_periods=10, high_col='High', low_col='Low', close_col='Close'):
    ...
    data['ch_vol_ema'] = data['ch_vol_hl'].ewm(
        ignore_na=False, min_periods=0, com=ema_periods, adjust=True).mean()
...
def trix(data, periods=14, signal_periods=9, close_col='Close'):
    data['trix'] = data[close_col].ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
    data['trix'] = data['trix'].ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
    data['trix'] = data['trix'].ewm(
        ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
    data['trix_signal'] = data['trix'].ewm(
        ignore_na=False, min_periods=0, com=signal_periods, adjust=True).mean()
...
def ultimate_oscillator(data, period_1=7, period_2=14, period_3=28, high_col='High', low_col='Low', close_col='Close'):
    ...
    for index, row in data.iterrows():
        if index > 0:
            ...
            bp = row[close_col] - \
                min(row[low_col], data.at[index-1, close_col])
            tr = max(row[high_col], data.at[index-1, close_col]) - \
                min(row[low_col], data.at[index-1, close_col])

            data.at[index, 'uo_bp'] = bp
            data.at[index, 'uo_tr'] = tr
            if index >= period_1:
                uo_avg_1 = sum(data['uo_bp'][index-period_1:index]) / \
                    sum(data['uo_tr'][index-period_1:index])
                data.at[index, 'uo_avg_1'] = uo_avg_1
            if index >= period_2:
                uo_avg_2 = sum(data['uo_bp'][index-period_2:index]) / \
                    sum(data['uo_tr'][index-period_2:index])
                data.at[index, 'uo_avg_2'] = uo_avg_2
            if index >= period_3:
                uo_avg_3 = sum(data['uo_bp'][index-period_3:index]) / \
                    sum(data['uo_tr'][index-period_3:index])
                data.at[index, 'uo_avg_3'] = uo_avg_3
                uo = (4 * uo_avg_1 + 2 * uo_avg_2 + uo_avg_3) / 7
                data.at[index, 'ultimate_oscillator'] = uo

    data = data.drop(['uo_bp', 'uo_tr', 'uo_avg_1',
                     'uo_avg_2', 'uo_avg_3'], axis=1)
